...
export function graduate(agentId, sessionMemory) {
  ...
  for (const entry of entries) {
    // Graduation rules — any reusable entry graduates
    // Previous threshold (roundtrips > 2) was too strict since roundtrips
    // are never auto-tracked, causing most decisions to never graduate
    const shouldGraduate = entry.reusable && (
      entry.type === 'ERROR' || // errors always graduate if reusable
      entry.type === 'DECISION' || // decisions always graduate if reusable
      entry.type === 'DISCOVERY' // discoveries always graduate if reusable
    );
