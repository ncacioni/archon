...
export function estimate(config) {
  ...
  // Only apply scout savings if actual cache entries exist (not speculative)
  const scoutSavings = (config.scoutCacheSize || 0) > 0
    ? Math.min(config.cachedPackages || 0, config.scoutCacheSize) * SCOUT_SAVING
    : 0;
