...
import { loadAgent, getTeamAgents, buildPrompt, getMode, getActiveAgents } from '../agent-registry.js';
...
describe('agent-registry', () => {
  ...
  describe('solo mode', () => {
    it('getMode() returns solo or team', () => {
      const mode = getMode();
      assert.ok(['solo', 'team'].includes(mode));
    });

    it('getActiveAgents() returns solo roster in solo mode', () => {
      const agents = getActiveAgents();
      assert.ok(Array.isArray(agents));
      assert.ok(agents.length >= 1);
    });

    it('loadAgent() loads a solo agent by ID', () => {
      const result = loadAgent('S2', {});
      assert.ok(result.prompt.length > 0, 'solo agent S2 should have a prompt');
      assert.ok(result.config.isMainAgent === false || result.config.isMainAgent === true);
    });

    it('loadAgent() returns empty for nonexistent solo agent', () => {
      const result = loadAgent('S9', {});
      assert.equal(result.prompt, '');
    });
  });
